...
| **API keys** | `exchangeApiKey`, `listClientApiKeys`, `createClientApiKey`, `revokeClientApiKey` — the three management calls take either an admin token or a user's own JWT ([self-service](/docs/features/api-keys#self-service-a-user-managing-their-own-keys)) |

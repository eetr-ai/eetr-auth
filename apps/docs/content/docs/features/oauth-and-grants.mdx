...
- If every requested scope is already consented to, the consent screen is **skipped** and the user
  goes straight back to the client.
- If the request adds a scope the user has not seen before, the screen is shown and lists **only the
  new scopes** — not everything already agreed to.
- `prompt=consent` always shows the screen, even when nothing new is being requested.

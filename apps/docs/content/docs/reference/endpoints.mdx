...
| GET/POST | `/api/admin/clients/[clientId]/api-keys` | List / issue a client's API keys &nbsp;<sup>†</sup> |
| DELETE | `/api/admin/clients/[clientId]/api-keys/[keyId]` | Revoke an API key &nbsp;<sup>†</sup> |
| POST | `/api/admin/site-logo` | Stage the site logo; applied when Site identity is saved |

<sup>†</sup> Also accepts a user-scoped JWT issued by the client in the path, confined to
that token's own user — see [self-service API keys](/docs/features/api-keys#self-service-a-user-managing-their-own-keys).

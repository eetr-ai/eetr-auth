...
5. **Empty vs filtered** — `EmptyState` with the header's CTA when the collection is genuinely empty;
...
6. **Create and edit** — one `SidePanel`, titled "New X" / "Edit X", footer "Add X" / "Save X" plus
   Cancel.
7. **Errors** — page-level `Banner` for list errors, in-panel `Banner` for save errors. Render only
   one at a time; a page-level banner is invisible behind the scrim.
8. **State** — `panelOpen`, `editingId`, `draft`, `baseline`, `saving`, `confirmingDeleteId`,
   `deletingId`.

 ...
 * Theme drift guard.
 *
 * The app has a two-tier theme (apps/auth/src/app/theme.css): a palette, and
 * semantic roles that components reference. That only stays true if raw
 * utilities cannot creep back in, so fail the build when they do.
 *
 * Run via `npm run lint` in apps/auth, or directly:
...
const SCAN = ["apps/auth/src/app", "apps/auth/src/components"];
...
if (failures > 0) {
	console.error(
		`\n${failures} theme violation(s). See apps/docs/content/docs/contributing/ux-guidelines.mdx › Theme.\n`,
	);
	process.exit(1);
}
